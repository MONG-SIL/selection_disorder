...
export default function OnboardingPreferences({ onComplete }) {
  ...
  // 음식별 선호도 평가를 저장하는 함수
  // foodId: 평가할 음식의 ID, rating: 1~5 점수
  const handleRating = (foodId, rating) => {
    setRatings({ ...ratings, [foodId]: rating });
  };
...
const handleFinish = async () => {
  const data = {
    ...
    categories,
    ratings,
    customFoods: customFoods.split(",").map(f => f.trim()).filter(f => f),
  };
    console.log("최종 취향 데이터:", data);
    try {
    const res = await axios.post("http://localhost:4000/api/user/onboarding-preferences", data);
    console.log("서버 저장 결과:", res.data);
    if (onComplete) onComplete(data); // 프론트에서도 상태 업데이트
  } catch (err) {
    console.error("온보딩 취향 저장 실패:", err);
  }
    // 회원가입 직후 → 서버로 POST 요청 예시
    // axios.post("/api/user/onboarding-preferences", data)
